...
3.  **Core System Telemetry:** 60-second updates of CPU Temperature, CPU Usage percentage, RAM percentage and absolute usage (Used & Free in MB), root disk percentage utilization and absolute usage (Used & Free in GB), system load averages (1m, 5m, 15m), network interface bandwidth (RX/TX kB/s rates), and SBC active hardware health (under-voltage detection and thermal throttling).
