...
# Description: Distro-agnostic post-installation script for sysmqttd
...
main() {
  ...
  if [ -d /run/systemd/system ]; then
      ...
      if [ "$IS_UPGRADE" = "true" ]; then
          ...
          systemctl restart sysmqttd.service >/dev/null 2>&1 || :
          # Exit silently since it is an upgrade (no diagnostic/success messages)
          exit 0
      fi
  fi
